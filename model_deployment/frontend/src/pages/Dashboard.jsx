...
const Dashboard = () => {
    ...
    return (
        <Layout>
            ...
            {!loading && inventory.filter(item => Number(item.quantity) <= 0.1).length > 0 && (
                <motion.div
                    initial={{ opacity: 0, y: -10 }}
                    animate={{ opacity: 1, y: 0 }}
                    className="mb-6 p-4 rounded-xl bg-gold-500/10 border border-gold-500/30 flex items-center gap-3"
                >
                    ...
                    <div className="flex-1">
                        <p className="text-gold-300 font-medium">
                            {inventory.filter(item => Number(item.quantity) <= 0.1).length} item{inventory.filter(item => Number(item.quantity) <= 0.1).length > 1 ? 's' : ''} running low
                        </p>
                        <p className="text-gold-400/70 text-sm">
                            {inventory.filter(item => Number(item.quantity) <= 0.1).slice(0, 3).map(i => i.item_name).join(', ')}
                            {inventory.filter(item => Number(item.quantity) <= 0.1).length > 3 ? ` +${inventory.filter(item => Number(item.quantity) <= 0.1).length - 3} more` : ''}
                        </p>
                    </div>
                </motion.div>
            )}
